                ...
                <div class="card card-primary card-outline">
                    ...
                    <div class="card-body">
                          <div class="table-responsive">
                              <table id="tabla-group" class="table table-bordered table-striped">
                                <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Grupo</th>
                                        ...
                                        <td>
                                            @if(is_null($items->deleted_at))
                                                ...
                                                <form style="display: inline" method="POST" action="{{ route('groups.destroy',['id'=>$items->group_id]) }}">
                                                    {!! method_field('DELETE') !!}
                                                    {!! csrf_field() !!}
                                                    <button type = "submit" name="eliminar" id="eliminar2" class="btn btn-danger btn-sm" data-toggle="tooltip" title="Ocultar grupo"><i class="fa fa-eye" aria-hidden="true"></i></button>
                                                </form>
                                            @else
                                                <form style="display: inline" method="POST" action="{{ route('groups.restore',['id'=>$items->group_id]) }}">
                                                    {!! csrf_field() !!}
                                                    <button type = "submit" name="eliminar" id="eliminar3" class="btn btn-default btn-sm" data-toggle="tooltip" title="Ocultar grupo"><i class="fa fa-eye-slash" aria-hidden="true"></i></button>
                                                </form>
                                            @endif
                                        </td>
                                    </tr>
